...
import { removeTokenFromLocalStorage, token } from "@/lib/api/baseUrl";
...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      document.removeEventListener("mousedown", handleClickOutsideSetting);
    };
  }, []);
  ...
  return (
    ...
    >
      <nav className="max-w-7xl mx-auto w-full px-4 md:px-8 flex gap-8 lg:justify-normal justify-between items-center">
        <Link href={"/sora/home"}>
          <Image height={55} width={100} src="/logoblack.png" alt="logo" />
        </Link>
          ...
          <div className="flex gap-4">
            ...
            {isAuthenticated ? (
              <div ref={dropdownSettingRef} className="relative">
                ...
                >
                  ...
                  <div className="h-10  w-10 rounded-full border border-gray-300">
                    <Image
                      src={user.user?.avatar! ? user.user?.avatar! : "/10.png"}
                      alt="user"
                      width={40}
                      height={40}
                ...
                {isOpenDrop && (
                  <div className="absolute top-11 bg-white left-4 shadow-lg border border-gray-200 rounded-md flex flex-col gap-2 px-4 py-2">
                    ...
                    >
                      <IoSettingsOutline size={18} /> Voir Profile
                    </button>
                    <button
                      className="flex items-center gap-1 text-sm font-medium text-customBlue hover:text-customHoverBlue"
                      type="button"
              ...
              <div className="flex flex-row gap-4">
                <SignButton text="S'inscrire" href="/signup" variant="light" />
                <SignButton text="Se connecter" href="/signin" variant="dark" />
              </div>
            )}
            {/* translations button */}
        ...
        <LuMenu
          ...
          size={30}
        />
        <NavMob isNavMobOpen={isNavMobOpen} setIsNavMobOpen={setIsNavMobOpen} />
      </nav>
    </header>
  );
};
